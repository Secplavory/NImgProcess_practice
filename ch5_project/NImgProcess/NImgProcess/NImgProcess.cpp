#include "Thresholding_src/INImgProcess.h"
...
void OtsuThreshold(NImage* pImg) {
	Threshold(pImg, &Otsu);
}

void KSW_EntropicThreshold(NImage* pImg) {
	Threshold(pImg, &KSW_Entropic);
}

void MomentThreshold(NImage* pImg) {
	Threshold(pImg, &Moment);
}

void Threshold(NImage *pImg, int (*thresholdFunc)(long* pg))
{
...
void Sobel(BYTE **list0,BYTE **list1,int Dx,int Dy)  //  Sobel 
{
